...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Inquiry Details') }} #{{ $inquiry->id }}</h1>
    </div>

    <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        <div class="bg-green-700 text-white px-6 py-4 flex items-center justify-between">
            <span class="font-bold">{{ __('Inquiry') }} #{{ $inquiry->id }}</span>
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <p class="text-sm text-gray-500 mb-1">{{ __('Inquiry Type') }}</p>
                    <p class="font-semibold text-gray-900">{{ $inquiry->type->name ?? '-' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">{{ __('Request Date') }}</p>
            ...
            @if($inquiry->result_file_path)
                <div>
                    <p class="text-sm text-gray-500 mb-2">{{ __('Result File') }}</p>
